...
def resolver_metodo_grafico(
    ...
    if bounds is None:
        bounds = {"x1_min": 0, "x1_max": None, "x2_min": 0, "x2_max": None}
    x1_min = bounds.get("x1_min")
    x1_max = bounds.get("x1_max")
    x2_min = bounds.get("x2_min")
    x2_max = bounds.get("x2_max")
    ...
    bound_val = BOUND
    for v in (x1_min, x1_max, x2_min, x2_max):
        if v is not None:
            bound_val = max(bound_val, abs(v) + 1)

    poly = _build_feasible_polygon(restr, bound=bound_val)
    ...
    lp_bounds = [(x1_min, x1_max), (x2_min, x2_max)]
    lp_res = None
    try:
        lp_res = linprog(
            c_vec,
    ...
    vertices = [
        {"x": float(x), "y": float(y), "z": float(val)}
        for (x, y), val in zip(candidates, values)
    ]

    status = "optimo"
    opt_segment = None
    if lp_res is not None and lp_res.status == 3:
        status = "no acotada"
        opt_points = []
        vertices = []
        opt_val = float("nan")
    elif len(opt_points) > 1:
        status = "multiple"
        xs = [p[0] for p in opt_points]
        ys = [p[1] for p in opt_points]
        opt_segment = [(min(xs), min(ys)), (max(xs), max(ys))]

    max_x = max((v["x"] for v in vertices), default=bound_val)
    max_y = max((v["y"] for v in vertices), default=bound_val)
    x_max = max(max_x, 0) + 1
    y_max = max(max_y, 0) + 1
    ...
    for a, b_, op, c in restr:
        ...
        fig.add_trace(
            go.Scatter(
                x=x_line, y=y_line, mode="lines", name=f"{a}x₁ + {b_}x₂ {op} {c}"
            )
        )
